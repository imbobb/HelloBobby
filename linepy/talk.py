...
class Talk(object):
    ...
    @loggedIn
    def sendReplyMention(self, msg_id, to, mid, firstmessage='', lastmessage=''):
        arrData = ""
        text = "%s " %(str(firstmessage))
        arr = []
        mention = "@zeroxyuuki "
        ...
        arr.append(arrData)
        text += mention + str(lastmessage)
        self.sendReplyMessage(msg_id, to, text, {'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    ...
    @loggedIn
    def sendReplyMentionV2(self, msg_id, to, text="", mids=[], isUnicode=False):
        arrData = ""
        arr = []
        mention = "@zeroxyuuki "
        if mids == []:
    ...
    def sendReplyImageWithURL(self, msg_id, to, url):
        path = self.downloadFileURL(url, 'path')
        return self.sendReplyImage(msg_id, to, path)
    ...
    @loggedIn
    def sendReplyGIF(self, to, path):
        return self.uploadObjTalk(path=path, type='gif', returnAs='bool', to=to)

    @loggedIn
    def sendReplyGIFWithURL(self, to, url):
        path = self.downloadFileURL(url, 'path')
        return self.sendReplyGIF(to, path)
